import { c, S as w, a as F, f as i, g as r, i as d, m as P, B as y, F as L, J as S, k as n } from "./index-D8luuwzV.js";
import { A as v } from "./AbstractID3Parser-CsfzmK6r.js";
const l = c("music-metadata:parser:musepack:sv8"), x = new w(2, "latin1"), u = {
  len: 5,
  get: (a, e) => ({
    crc: i.get(a, e),
...
class C {
  ...
  async readPacketHeader() {
    const e = await this.tokenizer.readToken(x), t = await this.readVariableSizeField();
    return {
      key: e,
...
class T extends y {
  ...
  async parse() {
    if (await this.tokenizer.readToken(L) !== "MPCK")
    ...
    do {
      ...
      switch (M(`packet-header key=${t.key}, payloadLength=${t.payloadLength}`), t.key) {
        ...
        case "CT":
          ...
          break;
        case "SE":
          return this.metadata.format.duration && this.metadata.setFormat("bitrate", this.audioLength * 8 / this.metadata.format.duration), S.tryParseApeHeader(this.metadata, this.tokenizer, this.options);
        default:
...
class V {
  constructor(e) {
    this.tokenizer = e, this.pos = 0, this.dword = null;
  }
  /**
   *
...
class b extends y {
  ...
  async parse() {
    ...
    this.metadata.setFormat("numberOfSamples", t), this.duration = t / e.sampleFrequency, this.metadata.setFormat("duration", this.duration), this.bitreader = new V(this.tokenizer), this.metadata.setFormat("numberOfChannels", e.midSideStereo || e.intensityStereo ? 2 : 1);
    const s = await this.bitreader.read(8);
    return this.metadata.setFormat("codec", (s / 100).toFixed(2)), await this.skipAudioData(e.frameCount), k(`End of audio stream, switching to APEv2, offset=${this.tokenizer.position}`), S.tryParseApeHeader(this.metadata, this.tokenizer, this.options);
  }
...
class O extends v {
  async postId3v2Parse() {
    ...
    switch (e) {
      case "MP+": {
        g("Stream-version 7"), t = new b(this.metadata, this.tokenizer, this.options);
        break;
      }
      case "MPC": {
