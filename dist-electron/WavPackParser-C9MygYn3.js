import { f as l, F as u, k as D, d as I, c as f, B as z, J as B, N as S, a as k, u as w, m as x } from "./index-D8luuwzV.js";
const P = [
  6e3,
  8e3,
  9600,
...
const o = f("music-metadata:parser:WavPack");
...
class F extends z {
  ...
  async parse() {
    return this.audioDataSize = 0, await this.parseWavPackBlocks(), B.tryParseApeHeader(this.metadata, this.tokenizer, this.options);
  }
  ...
  async parseMetadataSubBlock(t, a) {
    let s = a;
    for (; s > m.len; ) {
      const n = await this.tokenizer.readToken(m), h = await this.tokenizer.readNumber(n.largeBlock ? S : k), r = new Uint8Array(h * 2 - (n.isOddSize ? 1 : 0));
      switch (await this.tokenizer.readBuffer(r), o(`Metadata Sub-Blocks functionId=0x${n.functionId.toString(16)}, id.largeBlock=${n.largeBlock},data-size=${r.length}`), n.functionId) {
        ...
        case 14: {
          o("ID_DSD_BLOCK");
          const b = 1 << k.get(r, 0), p = t.flags.samplingRate * b * 8;
          if (!t.flags.isDSD)
      ...
      s -= m.len + (n.largeBlock ? S.len : k.len) + h * 2, o(`remainingLength=${s}`), n.isOddSize && this.tokenizer.ignore(1);
    }
    if (s !== 0)
